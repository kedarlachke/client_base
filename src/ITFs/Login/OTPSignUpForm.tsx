...
import {ActionToDispatch,ActionToRedirect,handleMobileSignUpJWT,checkCurrentUsernameJWT,handleVerifyMobileOTPJWT} from '../Redux/reducers/actions'
...
const handleSubmit=(currentdocument:any)=>{
 ...
 let values={
   mobile, lastname,email,username:mobile, password, applicationid ,client,lang 
 }
...
const handleOTPSubmit=(currentdocument:any)=>{
 ...
 let { mobile,email,username, password, applicationid ,client,lang,otp}=currentdocument
 let values={
   mobile,email,username:mobile, password, applicationid ,client,lang,mobileotp:otp 
 }
...
{
  const {touched,username,mobile,password,email,validatemode,otp} = currentdocument ; 
  currentdocument.formValid=true
   let isNew=false;
   let username_check=runCheck(nvl(username,''), [requiredCheck]);
   let mobile_check=runCheck(nvl(mobile,''), [requiredCheck,numberCheck]);
  ...
  if(validatemode=='touch' && touched!=null)
   ...
   console.log(username_check.length===0 && otp_check.length===0 && password_check.length===0 && mobile_check.length === 0 && email_check.length === 0)
   if(username_check.length !== 0 || password_check.length !== 0 || mobile_check.length !== 0 || email_check.length !== 0){
    currentdocument.formValid=false
   }
  return currentdocument;
